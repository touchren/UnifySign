...
let { config, storage_name: _storage_name } = require("../config.js")(
  runtime,
  global
);
...
var canvasWindow = floaty.rawWindow(
  <vertical>
    <vertical
      id="vertical"
      bg="#aaaaaa"
      w="{{Math.floor(device_width*0.8)}}px"
      h="{{Math.floor(device_height*0.5)}}px"
      gravity="center"
    >
    ...
    <vertical
      ...
      bg="#aaaaaa"
      w="{{Math.floor(device_width*0.8)}}px"
      h="{{Math.floor(device_height*0.2)}}px"
      gravity="center"
    >
...
var canvasWindowCtrl = new FloatyController(
  canvasWindow,
  canvasWindow.btnMove,
  1,
  canvasWindow.vertical
);
...
miniWindowCtrl.setClick(function () {
  ...
  threads.start(function () {
    ...
    canvasWindowCtrl.windowMoving({
      from: { X: miniMove.to.X, Y: canvasMove.to.Y },
      to: canvasMove.from,
    });
  ...
  threads.start(function () {
    if (cutMode) {
      if (positions && positions.length === 4) {
        ...
        // setClip("data:image/png;base64," + base64Str);
        log("data:image/png;base64," + base64Str);
        previewImage && previewImage.recycle();
        previewImage = null;
      } else {
        toastLog("未框选小图");
  ...
  threads.start(function () {
    if (cutMode) {
      if (!paddleOcrUtil.enabled) {
        ...
        ui.post(() => {
          canvasWindow.recognize_text.text(
      ...
      if (positions && positions.length === 4) {
        ...
        if (clipImg.getHeight() < 80) {
          ...
          log(
            "缩放图片大小：" + clipImg.getWidth() + "," + clipImg.getHeight()
          );
        }
...
threads.start(function () {
  sleep(100);
  ui.post(() => {
    canvasWindow.setPosition(
      device_width / 2 - canvasWindow.getWidth() / 2,
      device_height / 2 - canvasWindow.getHeight() / 2
    );
  });
  sleep(100);
  data = {
...
function convertAndClip(positions, data, img) {
  let { left, right, top, bottom, width, height } = getRealRegion(
    positions,
    data,
    img
  );
      ...
      switch (
        ...
        case event.ACTION_MOVE:
          if (pointerCount == 1) {
            ...
            var X1 = event.getX(1);
            var Y1 = event.getY(1);
            var touchStartDistance = getDistance(
  ...
  var fileList = files
    .listDir(path, function (name) {
      return (
        name.endsWith(".jpg") ||
        name.endsWith(".png") ||
        (files.isDir(files.join(path, name)) && !name.startsWith("."))
      );
    })
    .sort();
...
function pointRangeBack(x, y, moveRangeInfo) {
  ...
  x =
    moveRangeInfo.from.X < x && x < moveRangeInfo.to.X
      ? x
      : moveRangeInfo.from.X < x
      ? moveRangeInfo.to.X
      : moveRangeInfo.from.X;
  y =
    moveRangeInfo.from.Y < y && y < moveRangeInfo.to.Y
      ? y
      : moveRangeInfo.from.Y < y
      ? moveRangeInfo.to.Y
      : moveRangeInfo.from.Y;
  return {
    x: x,
    y: y,
  };
...
function FloatyController(window, windowMoveId, isCanvasWinow) {
  ...
  setInterval(() => {
    if (context.resources.configuration.orientation != this.orientation) {
      ...
      this.width = this.orientation == 1 ? device_width : device_height;
      this.height = this.orientation == 2 ? device_width : device_height;
      var xy = pointRangeBack(
        window.getX(),
        window.getY(),
        this.moveRange(window)
      );
      this.windowMoving([
  ...
  if (windowMoveId) {
    windowMoveId.setOnTouchListener(
      new View.OnTouchListener((view, event) => {
          ...
          switch (event.getAction()) {
            ...
            case event.ACTION_MOVE:
              ...
              if (this.eventMoving && this.eventKeep) {
                ui.post(() => {
                  window.setPosition(
                    this.windowStartX + sx,
                    this.windowStartY + sy
                  );
                });
              }
              break;
    ...
    if (!isCanvasWinow) {
      if (view) {
        ...
        return convertArrayToReadable([
          [0, 0],
          [
            this.width - win.getWidth() + border * 2,
            this.height -
              win.getHeight() -
              barHeight +
              border * 2 -
      ...
      if (view) {
        return convertArrayToReadable([
          ...
          [
            this.width - (view.getX() + view.getWidth()),
            this.height - (view.getY() + view.getHeight()) - bottomHeight,
          ],
        ]);
      } else {
        return convertArrayToReadable([
          ...
          [
            ...
            this.height - win.getHeight() - bottomHeight,
          ],
        ]);
      }
    }
  ...
  this.viewIntoScreen = () => {
    ...
    var point = pointRangeBack(
      window.getX(),
      window.getY(),
      this.moveRange(window, windowMoveId)
    );
    return convertArrayToReadable([
      [window.getX(), window.getY()],
      [point.x, point.y],
