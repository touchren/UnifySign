...
let _debugInfo =
  typeof debugInfo === "undefined" ? (v) => console.verbose(v) : debugInfo;
let _errorInfo =
  typeof errorInfo === "undefined" ? (v) => console.error(v) : errorInfo;
  ...
  ui.run(function () {
    ...
    try {
      if (position && isFinite(position.x) && isFinite(position.y)) {
        _this.floatyWindow.setPosition(
          parseInt(position.x),
          parseInt(position.y) + _config.bang_offset
        );
      } else if (position && position.bounds()) {
        _this.floatyWindow.setPosition(
          parseInt(position.bounds().left + 10),
          parseInt(position.bounds().top - 35) + _config.bang_offset
        );
      }
      if (text) {
